...
def generate_launch_description():
    ...
    declared_arguments = [
        ...
        DeclareLaunchArgument("baseTrajectoryAmplitude", default_value=str(base_trajectory_defaults.get("amplitude", trajectory_defaults.get("amplitude", 0.2))), description="Base line-segment half-length (m)."),
        DeclareLaunchArgument("baseTrajectoryFrequency", default_value=str(base_trajectory_defaults.get("frequency", trajectory_defaults.get("frequency", 0.2))), description="Base line-segment oscillation frequency (Hz)."),
        DeclareLaunchArgument("baseTrajectoryYaw", default_value=str(base_trajectory_defaults.get("yaw", 0.0)), description="Base target yaw in world frame (rad)."),
        DeclareLaunchArgument("baseTrajectorySlope", default_value=str(base_trajectory_defaults.get("slope", 0.0)), description="Slope dy/dx of the base target line segment."),
    ...
    trajectory_node = Node(
        ...
        parameters=[{
            ...
            "baseYaw": LaunchConfiguration("baseTrajectoryYaw"),
            "baseSlope": LaunchConfiguration("baseTrajectorySlope"),
